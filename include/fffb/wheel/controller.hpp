...
public:
        ...
        constexpr bool   add_forces ( vector< force_data > const & forces ) noexcept ;
...
{
        FFFB_F_DBG_S( "controller", "looking for wheels..." ) ;
        ...
        for( auto & device : devices )
        {
                for( uti::ssize_t i = 0; i < known_wheel_device_ids.size(); ++i )
                {
                        ...
                        {
                                FFFB_F_DBG_S( "controller", "found wheel, device id 0x%.8x", device.device_id() ) ;
                                return wheel( device ) ;
                        }
                }
        }
...
{
        FFFB_F_DBG_S( "controller::calibrate", "starting calibration" ) ;
        ...
        FFFB_F_DBG_S( "controller::calibrate", "finished calibration" ) ;
...
{
        FFFB_F_DBG_S( "controller::disable_autocenter", "" ) ;
...
{
        FFFB_F_DBG_S( "controller::enable_autocenter", "" ) ;
...
{
        FFFB_F_DBG_S( "controller::set_autocenter", "" ) ;
...
{
        FFFB_F_DBG_S( "controller::add_force", "adding force of type %d", uti::to_underlying( force.type ) ) ;
...
constexpr bool controller::add_forces ( vector< force_data > const & forces ) noexcept
{
        for( auto const & force : forces )
        {
                if( !add_force( force ) ) return false ;
        }
        return true ;
}
...
{
        FFFB_F_DBG_S( "controller::clear_force", "clearing force of type %d", uti::to_underlying( type ) ) ;
        for( auto & force_data : wheel_.forces() )
        {
                if( force_data.type == type )
                {
                        force_data = {} ;
                        FFFB_F_DBG_S( "controller::clear_force", "force cleared" ) ;
                        return true ;
                }
        }
        FFFB_F_WARN_S( "controller::clear_force", "force not found" ) ;
        return false ;
}
...
constexpr bool controller::clear_forces () noexcept
{
        ...
        FFFB_F_DBG_S( "controller::clear_forces", "forces cleared" ) ;
        return true ;
}

constexpr bool controller::download_forces () const noexcept
{
        uti::u8_t force_count { 0 } ;

        for( uti::ssize_t i = 0; i < wheel_.forces().size(); ++i )
        {
                if( wheel_.forces().at( i ).type != force_type::NONE )
                {
                        force_count += _write_report( protocol_provider::download_force( wheel_.protocol(), ( 1 << i ), wheel_.forces().at( i ) ), "controller::download_forces" ) ;
                }
        }
        FFFB_F_DBG_S( "controller::download_forces", "downloaded %d forces", force_count ) ;
        return force_count ;
}
...
constexpr bool controller::play_forces () const noexcept
{
        ...
        FFFB_F_DBG_S( "controller::play_forces", "playing forces in slots 0x%.8x", slots ) ;
...
{
        FFFB_F_DBG_S( "controller::stop_forces", "" ) ;
        return _write_report( protocol_provider::stop_force( wheel_.protocol(), 0x0F ), "controller::stop_forces" ) ;
}

constexpr bool controller::set_led_pattern ( uti::u8_t const pattern ) const noexcept
{
        FFFB_F_DBG_S( "controller::set_led_pattern", "" ) ;
        return _write_report( protocol_provider::set_led_pattern( wheel_.protocol(), pattern ), "controller::set_led_pattern" ) ;
}
...
constexpr bool controller::_write_report ( report const & report, [[ maybe_unused ]] char const * scope ) const noexcept
{
        ...
        {
                FFFB_F_ERR_S( scope, "open_device failed on device %x", wheel_.device().device_id() ) ;
                return false ;
        }
        if( !wheel_.device().write( report ) )
        {
                FFFB_F_ERR_S( scope, "send_report failed on device %x", wheel_.device().device_id() ) ;
                wheel_.device().close() ;
                return false ;
        }
        if( !wheel_.device().close() )
        {
                FFFB_F_ERR_S( scope, "close_device failed on device %x", wheel_.device().device_id() ) ;
                return false ;
        }
        FFFB_F_DBG_S( scope, "write_report successful on device %x", wheel_.device().device_id() ) ;
        return true ;
}
